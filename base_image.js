...
let textFrame = { x: 0, y: 0, w: 0, h: 0 };function pickPalette() {
  let p = random(palette);
  if (!p || p.length < 2) {
    console.error("Invalid palette:", p);
    return;
  }
  darkColor = color(p[0]);
  lightColor = color(p[1]);
  if (!darkColor || !lightColor) {
    console.error("Invalid colors:", p[0], p[1]);
    return;
  }
...
function draw() {
  // 防御性检查，确保 darkColor 和 lightColor 已初始化
  if (!darkColor || !lightColor) {
    console.error("Colors not initialized. Skipping draw.");
    return;
  }
      ...
      fetch(url, {
        ...
        .then(data => {
          ...
          if (imageUrls.length > 0) {
            ...
            loadImage(selected, (loadedImg) => {
              img = loadedImg; // 修复：直接赋值给全局变量
              loaded = true;
              pickPalette();
              resolve();
            }, () => reject("圖片載入失敗"));
          } else reject("Imgur 相簿中沒有圖片");
        })
        .catch(err => reject("Imgur 載入失敗: " + err));
    });
  }
...
function setup() {
  preloadPromise.then(() => {
    ...
    textAlign(CENTER, CENTER);
    textSize(18);
    text("圖片載入失敗：" + err, width / 2, height / 2);
  });
}
